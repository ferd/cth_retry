...
post_end_per_testcase(_TC,_Config,ok,State) ->
    {ok, State};
post_end_per_testcase(TC,Config,Error,State=#state{suite=Suite, groups=Groups, acc=Acc}) ->
    Test = case TC of
        {_Group, Case} -> Case;
        TC -> TC
    end,
...
on_tc_skip(Suite, {TC,_Group}, _Reason, State) ->
...
on_tc_skip({TC,Group}, Reason, State) ->
    State;
...
terminate(#state{acc=Acc}) ->
    ...
    io:format(user, "Writing retry specification at ~s~n", [Path]),
